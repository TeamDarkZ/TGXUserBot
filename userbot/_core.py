...
DELETE_TIMEOUT = 5
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Tgx User"
Tgx_logo = "./Tgx/Tgx_bot_logo.jpg"
furious = TgxBot.uid
Tgx = f"[{DEFAULTUSER}](tg://user?id={furious})"

@TgxBot.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@TgxBot.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    omk = f"**⍟ Plugin name ≈** `{input_str}`\n**⍟ Uploaded by ≈** {Tgx}\n\n⚡ **[Legendary TgxBot](t.me/TgxUserbot)** ⚡"
...
@TgxBot.on(admin_cmd(pattern="install$", outgoing=True))
@TgxBot.on(sudo_cmd(pattern="install$", allow_sudo=True))
...
@TgxBot.on(admin_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
@TgxBot.on(sudo_cmd(pattern=r"uninstall (?P<shortname>\w+)", allow_sudo=True))
...
@TgxBot.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
@TgxBot.on(sudo_cmd(pattern=r"upload (?P<shortname>\w+)$", allow_sudo=True))
...
@TgxBot.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
@TgxBot.on(sudo_cmd(pattern=r"load (?P<shortname>\w+)$", allow_sudo=True))
