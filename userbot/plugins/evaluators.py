...
async def _(event):
    ...
    Tgxevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_sTgx(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    Tgxuser = await event.client.get_me()
    if Tgxuser.username:
        curruser = Tgxuser.username
    else:
        curruser = "TgxBot"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    ...
    Tgxevent = await edit_or_reply(event, "`Running ...`")
