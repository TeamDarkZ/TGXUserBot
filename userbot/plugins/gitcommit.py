...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of TgxBot`")
        return
    TgxBot = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await TgxBot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, TgxBot)


async def git_commit(file_name, TgxBot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await TgxBot.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await TgxBot.edit("Cannot Upload Plugin")
    else:
        return await TgxBot.edit("`Committed Suicide`")
