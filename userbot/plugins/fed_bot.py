...
async def _(event):
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await event.client.send_message(chat, f"/newfed {Tgx_input}")
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
          try:     
              ...
              await event.client.send_message(chat, f"/renamefed {Tgx_input}")
              response = await response 
          except YouBlockedUserError: 
...
async def _(event):
    if event.fwd_from:
        return
    Tgx = await event.edit("`Collecting fstat....`")
    thumb = Tgx_logo
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
            ...
            try:
                ...
                if "Looks like" in massive.text:
                    ...
                    await TgxBot.send_file(
                        ...
                        massive,
                        thumb=thumb,
                        caption=f"List of feds {user} has been banned in.\n\n**⚡ [Collected using TgxBot](t.me/TgxUserbot) ⚡**",
                    )
                else:
                    await borg.send_message(event.chat_id, massive.text)
                await event.delete()
            except YouBlockedUserError:
                await Tgx.edit("`Please Unblock` @MissRose_Bot")
...
async def _(event):
    if event.fwd_from:
        return
    Tgx = await eor(event, "`Fetching fed info.... please wait`")
    lavde = event.pattern_match.group(1)
    async with borg.conversation(bot) as conv:
        try:
            await conv.send_message("/start")
