...
async def _(Tgx):
    if Tgx.fwd_from:
        return
    reply_to_id = Tgx.message.id
    if Tgx.reply_to_msg_id:
        reply_to_id = Tgx.reply_to_msg_id
    event = await edit_or_reply(Tgx, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await Tgx.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await Tgx.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    Tgxreply = await event.get_reply_message()
    if not Tgxreply or not Tgxreply.media or not Tgxreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if Tgxreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    Tgxevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, Tgxreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await Tgxevent.edit("`This file is not supported`")
            Tgxresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            Tgxfile = Path(await event.client.download_media(Tgxresponse, "./temp/"))
            Tgxgif = Path(await unzip(Tgxfile))
            ...
            for files in (Tgxgif, Tgxfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await Tgxevent.edit("Unblock @tgstogifbot")
            return
