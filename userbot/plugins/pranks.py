...
async def TgxBot(Tgxmemes):
    ...
    if not replied:
        await edit_or_reply(
            Tgxmemes, "`Media file not supported. Reply to a supported media`"
        )
        return
    if replied.media:
        Tgxmemmes = await edit_or_reply(Tgxmemes, "`Detecting Threats.........`")
    else:
        await edit_or_reply(
            Tgxmemes, "`Media file not supported. Reply to a suported media`"
        )
        return
    try:
        ...
        pass
    download_location = await Tgxmemes.client.download_media(replied, "./temp/")
    if download_location.endswith((".webp")):
        download_location = convert_toimage(download_location)
    size = os.stat(download_location).st_size
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            await Tgxmemmes.edit(
                "`The replied file is not supported. It should be less than 5mb -_-`"
            )
            os.remove(download_location)
            return
        await Tgxmemmes.edit("`Detected Threats....`")
    else:
        await Tgxmemmes.edit("`the replied file is not supported`")
        os.remove(download_location)
        return
    try:
        ...
        await Tgxmemmes.edit("ERROR: " + str(exc))
        os.remove(download_location)
        return
    Tgx = f"https://telegra.ph{response[0]}"
    Tgx = await threats(Tgx)
    await Tgxmemmes.delete()
...
async def TgxBot(Tgxmemes):
    ...
    if not replied:
        ...
        return
    if replied.media:
        Tgxmemmes = await edit_or_reply(Tgxmemes, "`Detecting Trash....`")
    else:
        await edit_or_reply(
            Tgxmemes, "`Media file not supported. Reply to a suported media`"
        )
    ...
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            ...
            os.remove(download_location)
            return
        await Tgxmemmes.edit("`Detected Trash.....`")
    else:
        await Tgxmemmes.edit("Media file not supported. Reply to a suported media")
        os.remove(download_location)
        return
    try:
        response = upload_file(download_location)
...
async def TgxBot(Tgxmemes):
    input_str = Tgxmemes.pattern_match.group(1)
    ...
    if not replied:
        ...
        return
    if replied.media:
        Tgxmemmes = await edit_or_reply(Tgxmemes, "`Trapping.....`")
    else:
        await edit_or_reply(
            Tgxmemes, "Media file not supported. Reply to a suported media"
        )
    ...
    try:
        ...
        return
    Tgx = f"https://telegra.ph{response[0]}"
    Tgx = await trap(text1, text2, Tgx)
    await Tgxmemmes.delete()
    await Tgxmemes.client.send_file(Tgxmemes.chat_id, Tgx, reply_to=replied)
...
async def TgxBot(Tgxmemes):
    ...
    if not replied:
        ...
        return
    if replied.media:
        Tgxmemmes = await edit_or_reply(Tgxmemes, "`Making A Comment`.")
    else:
        await edit_or_reply(
            Tgxmemes, "Media file not supported. Reply to a suported media"
        )
    ...
    if download_location.endswith((".jpg", ".jpeg", ".png", ".bmp", ".ico")):
        if size > 5242880:
            ...
            os.remove(download_location)
            return
        await Tgxmemmes.edit("Commented....")
    else:
        await Tgxmemmes.edit("Media file not supported. Reply to a suported media")
        os.remove(download_location)
        return
    try:
        ...
        return
    Tgx = f"https://telegra.ph{response[0]}"
    Tgx = await phcomment(Tgx, text, username)
    await Tgxmemmes.delete()
    await Tgxmemes.client.send_file(Tgxmemes.chat_id, Tgx, reply_to=replied)
...
CmdHelp("prank").add_command(
  ...
  "prank", None, "If this module doesn't work then contact admins in @TgxSupport"
).add()