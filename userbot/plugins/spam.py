...
async def bigspam(Tgx):
    if not Tgx.text[0].isalpha() and Tgx.text[0] not in ("/", "#", "@", "!"):
        Tgx_msg = Tgx.text
        TgxBot_count = int(Tgx_msg[9:13])
        Tgx_spam = str(Tgx.text[13:])
        for i in range(1, TgxBot_count):
            await Tgx.respond(Tgx_spam)
        await Tgx.delete()
        if LOGGER:
            await Tgx.client.send_message(
