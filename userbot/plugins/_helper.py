...
async def yardim(event):
    ...
    if tgbotusername is not None or Tgx_input == "text":
        results = await event.client.inline_query(tgbotusername, "@TgxUserbot")
...
async def info(event):
    ...
    if input_str == "text":
        ...
        for i in sorted(SUDO_LIST):
            ...
            string += "\n"
        if len(string) > 4095:
            data = string.format(count=Tgxcount, plugincount=plugincount)
            key = (
                requests.post(
            ...
            return
        await event.reply(
            string.format(count=Tgxcount, plugincount=plugincount), link_preview=False
        )
        return
    if input_str:
        if input_str in SUDO_LIST:
            ...
            for i in SUDO_LIST[input_str]:
                ...
                Tgxcount += 1
            await event.reply(
                string.format(count=Tgxcount, input_str=input_str), parse_mode="HTML"
            )
        else:
        ...
        for i in sorted(SUDO_LIST):
            ...
            string += " "
            Tgxcount += 1
        await event.reply(string.format(count=Tgxcount), parse_mode="HTML")
