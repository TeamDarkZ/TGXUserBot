...
Harsh = str(ALIVE_NAME) if ALIVE_NAME else "Tgx User"
papa = borg.uid
...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        ...
        TgxBot = await edit_or_reply(ids, "`Ok! Gbaning this piece of shit....`")
    hum = await userbot.client.get_me()
    await TgxBot.edit(f"`🔥Global Ban Iz Cumin'💦.... Wait and watch nigga🚶`")
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await TgxBot.edit(f"Gbaning This retard🚶\n\nTotal Chats :- `{a}`")
            except:
                b += 1
    else:
    ...
    try:
        if gmute(user.id) is False:
            return await TgxBot.edit(f"**Error! User already gbanned.**")
    except:
        pass
    return await TgxBot.edit(
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await TgxBot.edit(f"Ok! Now Ungbaning this kiddo.\nChats:- `{a}`")
            except:
                b += 1
    else:
